...
[assembly: Xamarin.Forms.Dependency(typeof(OneMinuteMystery.iOS.BingSpeechApi))]
namespace OneMinuteMystery.iOS
{
	...
	public class BingSpeechApi : IBingSpeechApi
	{
		...
		public async Task<SpeechResult> SpeechToTextAsync(string appId, string key)
		{
			return await Task.Run(() => SpeechToText(appId, key));
		}

		public SpeechResult SpeechToText(string appId, string key)
		{
			AccessTokenInfo token;
			string headerValue;
			...
			using (fs = new FileStream(audioFile, FileMode.Open, FileAccess.Read))
			{
				...
				using (WebResponse response = request.GetResponse())
				{
					...
					root.results[0] = GetIntent(root.results[0]);
